...
public class RoleController {
    ...
    public ResponseEntity<Role> lookup(@RequestParam long roleId) {
        return this.roleService.lookUp(roleId);
    }

    @GetMapping(path = "lookup/name")
    public ResponseEntity<Role> lookup(@RequestParam String roleName) {
        return this.roleService.lookUp(roleName);
    }

    @PutMapping(path = "disable/name")
    public ResponseEntity<Boolean> disable(@RequestParam String name) {
        return this.roleService.disableRole(name);
    }

    @PutMapping(path = "disable/id")
    public ResponseEntity<Boolean> disable(@RequestParam long id) {
        return this.roleService.disableRole(id);
    }

    @PutMapping(path = "enable/id")
    public ResponseEntity<Boolean> enable(@RequestParam long id) {
        return this.roleService.enableRole(id);
    }

    @PutMapping(path = "enable/name")
    public ResponseEntity<Boolean> enable(@RequestParam String name) {
